...
- Do NOT classify as generate_quiz unless the teacher explicitly asks for quiz/test/assessment/worksheet/practice problems.
...
function extractBasicParams(message: string): Record<string, any> {
  ...
  const topic = lower
    ...
    .replace(/\b(?:quiz|test|assessment|exit ticket|exam|flashcards?|lesson|sub plan|substitute|iep|audio|podcast|email|report|worksheet|work sheet|practice sheet|practice problems)\b/g, ' ')
...
function inferLessonFollowUpIntent(
  ...
  if (
    /\b(?:quiz|test|assessment|worksheet|practice sheet|flashcards?|iep|sub(?:stitute)?|weekly prep|calendar|schedule|planner|export|download|pdf|pptx|powerpoint|slides)\b/i.test(
      normalized
    )
  ) {
    return null;
...
function ruleBasedIntent(
  ...
  }
  {
    ...
    const hasQuizKeyword = /\b(?:quiz|test|assessment|exit ticket|exam|question set|worksheet|work sheet|practice sheet|practice problems)\b/i.test(normalized);
  ...
  if (isBrief && /\b(?:quiz|test|assessment|exit ticket|exam|worksheet|work sheet|practice sheet)\b/i.test(trimmed)) {
    return { type: 'generate_quiz', confidence: 0.72, extractedParams: extractBasicParams(trimmed), reasoning: 'Matched brief quiz/worksheet keyword heuristic' };
  }
