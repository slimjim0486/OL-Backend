...
/**
 * Stripe webhook for DTC package purchases
 * POST /api/webhooks/stripe-dtc
 *
 * Handles:
 * - checkout.session.completed (one-time payment for packages)
 * - invoice.paid (Weekly Box recurring payments)
 * - customer.subscription.deleted (Weekly Box cancellation)
 * - payment_intent.payment_failed
 */
router.post('/stripe-dtc', async (req: Request, res: Response) => {
  const signature = req.headers['stripe-signature'] as string;

  if (!signature) {
    logger.warn('Stripe DTC webhook received without signature');
    return res.status(400).json({ error: 'Missing stripe-signature header' });
  }
  ...
  try {
    event = stripeService.constructWebhookEvent(req.body, signature, 'dtc');
  } catch (err: any) {
    logger.error('Stripe DTC webhook signature verification failed', { error: err.message });
    return res.status(400).json({ error: `Webhook signature verification failed: ${err.message}` });
  }

  logger.info('Stripe DTC webhook received', { type: event.type, id: event.id });

  try {
    // Lazy-import to avoid circular dependency at module load
    const { packagePurchaseService } = await import('../services/teacher/packagePurchaseService.js');

    switch (event.type) {
      case 'checkout.session.completed': {
        const session = event.data.object as Stripe.Checkout.Session;
        logger.info('DTC checkout session completed', { sessionId: session.id, mode: session.mode });
        await packagePurchaseService.handlePaymentCompleted(session);
        break;
      }

      case 'invoice.paid': {
        const invoice = event.data.object as Stripe.Invoice;
        await packagePurchaseService.handleRecurringPayment(invoice);
        break;
      }

      case 'customer.subscription.deleted': {
        const subscription = event.data.object as Stripe.Subscription;
        await packagePurchaseService.handleSubscriptionCancelled(subscription);
        break;
      }

      case 'payment_intent.payment_failed': {
        const paymentIntent = event.data.object as Stripe.PaymentIntent;
        logger.warn('DTC payment failed', { paymentIntentId: paymentIntent.id });
        break;
      }

      default:
        logger.debug('Unhandled Stripe DTC event type', { type: event.type });
    }

    res.status(200).json({ received: true });
  } catch (error) {
    logger.error('Error processing Stripe DTC webhook', { error, eventType: event.type });
    res.status(200).json({ received: true, error: 'Processing error logged' });
  }
});
