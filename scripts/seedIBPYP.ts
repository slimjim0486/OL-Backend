 ...
 * - Social Studies: 4 age bands (~150 standards)
 *
 ...
 *   --social-studies-only Seed only Social Studies standards
 */
...
import {
  ibPYPSocialStudiesCurriculum,
  getAllIBPYPSocialStudiesStandards,
  IBPYPSocialStudiesStandard,
  IBPYPSocialStudiesAgeBand
} from '../src/config/ibPYPSocialStudies';
...
function getConceptualArea(strand: string, subject: string): string {
  const mapping: Record<string, string> = {
    ...
    'Human Systems and Economic Activities': 'Economics',
    'Social Organization and Culture': 'Culture',
    'Continuity and Change Through Time': 'History',
    'Human and Natural Environments': 'Geography',
    'Resources and the Environment': 'Environment'
  };
...
function getAncestorDescriptions(strand: string, subject: string): string[] {
  ...
  if (subject === 'SOCIAL_STUDIES') {
    if (strand === 'Human Systems and Economic Activities') {
      return ['Social Studies', 'Economics'];
    } else if (strand === 'Social Organization and Culture') {
      return ['Social Studies', 'Culture'];
    } else if (strand === 'Continuity and Change Through Time') {
      return ['Social Studies', 'History'];
    } else if (strand === 'Human and Natural Environments') {
      return ['Social Studies', 'Geography'];
    } else if (strand === 'Resources and the Environment') {
      return ['Social Studies', 'Environment'];
    }
...
interface SeedOptions {
  ...
  englishOnly?: boolean;
  scienceOnly?: boolean;
  socialStudiesOnly?: boolean;
}

async function seedIBPYP(options: SeedOptions = {}) {
  const { mathOnly, englishOnly, scienceOnly, socialStudiesOnly } = options;
  const seedAll = !mathOnly && !englishOnly && !scienceOnly && !socialStudiesOnly;
  ...
  if (seedAll || socialStudiesOnly) {
    console.log(`  Social Studies: ${getAllIBPYPSocialStudiesStandards().length} standards (4 age bands)`);
  }
  ...
  try {
    ...
    if (seedAll || socialStudiesOnly) {
      console.log('\n[5/5] Seeding Social Studies standards...');
      const socialStudiesResult = await seedSocialStudiesSubject(jurisdiction.id);
      totalCreated += socialStudiesResult.created;
      totalUpdated += socialStudiesResult.updated;
    }
...
// Seed Social Studies (age-band-based)
async function seedSocialStudiesSubject(jurisdictionId: string): Promise<{ created: number; updated: number }> {
  let totalCreated = 0;
  let totalUpdated = 0;

  console.log(`\n  Standards by Age Band (Social Studies):`);
  ibPYPSocialStudiesCurriculum.ageBands.forEach((a: IBPYPSocialStudiesAgeBand) => {
    console.log(`    ${a.ageBandLabel}: ${a.standards.length} standards`);
  });
  ...
  for (const ageBandData of ibPYPSocialStudiesCurriculum.ageBands) {
    const setCode = `IB.PYP.${ageBandData.ageBand}.SS`;
    ...
    const standardSet = await prisma.standardSet.upsert({
      ...
      update: {
        title: `IB PYP Social Studies - ${ageBandData.ageBandLabel}`,
        subject: SUBJECT_MAP['SOCIAL_STUDIES'],
        educationLevels: [ageBandData.gradeEquivalent],
        gradeLevel: parseInt(ageBandData.ageBand.replace('A', '')),
        ageRangeMin: ageBandData.ageRangeMin,
        ageRangeMax: ageBandData.ageRangeMax,
        documentTitle: `IB PYP Social Studies Scope and Sequence - ${ageBandData.ageBandLabel}`,
        documentYear: '2008'
      },
      create: {
        jurisdictionId,
  ...
  for (const ageBandData of ibPYPSocialStudiesCurriculum.ageBands) {
    ...
    // Group by strand
    const strandGroups = ageBandData.standards.reduce((groups: Record<string, IBPYPSocialStudiesStandard[]>, std: IBPYPSocialStudiesStandard) => {
      if (!groups[std.strand]) {
        groups[std.strand] = [];
      }
      groups[std.strand].push(std);
      return groups;
    }, {} as Record<string, IBPYPSocialStudiesStandard[]>);
      ...
      for (const std of standards) {
        const result = await prisma.learningStandard.upsert({
          ...
          update: {
            description: std.description,
            strand: std.strand,
            conceptualArea: getConceptualArea(std.strand, 'SOCIAL_STUDIES'),
            isStatutory: true,
            position,
            depth: 0,
            ancestorDescriptions: getAncestorDescriptions(std.strand, 'SOCIAL_STUDIES')
          },
          create: {
            standardSetId,
            notation: std.notation,
...
const options: SeedOptions = {
  ...
  socialStudiesOnly: args.includes('--social-studies-only')
};
